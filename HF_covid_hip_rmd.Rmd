...
# Annual trends
e_hip_ip_unplanned_week_number_wide <-
  e_hip_ip_unplanned_week_number %>% 
  filter(year == 2020) %>% 
...
As of January 2021, increased waiting times for hip replacements resulted in a loss of **28,893 QALYs**; **12 thousand** lost QALYs were a direct result of reduced quality of life while waiting for treatment, while **16 thousand** lost QALYs represent worst patient outcomes (pain, reduced functioning etc) as a result of delayed treatment. Converting this figure to a monetary value, at **£20k per QALY** and discounting by **3.5%** per year, values the total loss at **£478  million**. In addition, the disutility of increased waiting can be estimated at **£27 million**.
...
<br>
Fig. 4.4
``` {r Unplanned hip replacement - cumulative deficit by region, warning = FALSE, message = FALSE, echo = FALSE}
e_hip_unplanned_def_region %>%
  filter(week_number < 53) %>% 
  mutate(label = if_else(week_number == max(week_number), as.character(demographic_var), NA_character_)) %>% 
  ggplot(aes(x = week_number, y = count, colour = demographic_var)) +
...
<br>
The quality of life implications relating to hip replacement surgery are impactful for the patient in both the pre- and post-operative stages of their care. Delays to care are thought to be more substantial in the pre-op phase but only last until surgery is provided; inversely, diminished quality of life in post-surgical patients is much less but may last for the remainder of their lives. Details below:

**Pre-operative impact on Quality of life**

The methodology for calculating the pre-surgery impacts of delayed hip replacements is as follows:

- Number of additional weeks waiting for hip replacements: `r paste(round(f_cum_add_weeks_waiting,0), "weeks")` 
- The number of additional years waiting is (1,444,251 / 52): `r paste(round(f_cum_add_years_waiting,0), "years")`
- Average EQ5D prior to hip surgery: `r f_eq5d_pre_op`
- Average EQ5D after to hip surgery: `r f_eq5d_post_op`
- EQ5D gain following hip replacement (0.802 – 0.342): `r f_eq5d_gain`
    - These are taken from <https://digital.nhs.uk/data-and-information/publications/statistical/patient-reported-outcome-measures-proms/hip-and-knee-replacement-procedures-april-2019-to-march-2020>.  
    - EQ5D runs from 0 to 1 with 1 being a state of perfect health and 0 being dead - although it is technically possible to have a state less then zero.  
    - Living a year with EQSD=1 is 1 QALY, as is living two years with EQ5D=0.5
- The pre-operative QALY loss of waiting `r paste(round(f_cum_add_weeks_waiting,0), "weeks")` additional weeks for hip replacements is `r paste(round(f_cum_add_years_waiting,0), "years")` x `r f_eq5d_gain` = `r paste(round(f_qaly_loss_preop,0), "QALYs")`


**Post-operative impact on Quality of life**

- Literature derived [2] post-surgical QALY impact of 1 week delay in hip replacement surgery: `r f_qaly_loss_week_postop`
- Total admission deficit: `r paste(e_hip_ip_total_admission_def, "admissions")`
- Cumulative additional weeks waiting for hip replacement: `r paste(f_cum_add_weeks_waiting, "weeks")`
- QALYs lost by those waiting for hip procedures: `r paste(round(f_qaly_loss_postop,1), "QALYs")`
- Lifetime QALYs lost by those waiting for hip procedures: `r paste(round(f_qaly_loss_lifetime_postop,1), "QALYs")`


**Monetary value of combined lost QALYs**

- Combined QALYs lost (pre- and post-surgery): **28,894 QALYs = 12776 + 16118**
- Monetary value of lifetime lost of QALYs (discounted by 3.5%) :  **£477.6m = 20000 x (12776 + 16118 x (12.4 / 18.0))**
- Disutility of waiting for hip procedures: **£27.4m**
...
<br>
<br>
These figures will continue to rise until the deficit is reduced, that is we recover all the activity lost due to covid-19. 
...
<br>
```{r London-style recovery, warning = FALSE, message = FALSE, echo = FALSE}
LTR_union %>% 
...
<br>
<br>
```{r London-style recovery table, warning = FALSE, message = FALSE, echo = FALSE}
create_dt(
LTR_union_diff_london %>%
